...
// Magic-byte (0xAD) delimiter. As a narrow literal on a POSIX UTF-8 locale it would fail wxConvLibc
// and collapse to empty, breaking operator-token comparisons in SearchList. Keep wxT() so the
// literal is wide-char at compile time -- same reason as FileTags.h's TAG_* macros.
