...
void CServerUDPSocket::OnPacketReceived(uint32 serverip, uint16 serverport, uint8_t *buffer, size_t length)
{
	...
	if (pServer && thePrefs::IsServerCryptLayerUDPEnabled() &&
		((pServer->GetServerKeyUDP() != 0 && pServer->SupportsObfuscationUDP()) ||
			(pServer->GetCryptPingReplyPending() && pServer->GetChallenge() != 0))) {
		// eMule TODO
		uint32 dwKey = 0;
		if (pServer->GetCryptPingReplyPending() &&
			pServer->GetChallenge() !=
				0 /* && pServer->GetPort() == ntohs(sockAddr.sin_port) - 12 */) {
		...
		wxASSERT(dwKey != 0);
		nPayLoadLen = CEncryptedDatagramSocket::DecryptReceivedServer(
			buffer, length, &pBuffer, dwKey, serverip);
#ifdef __DEBUG__
		if (nPayLoadLen == length) {
			AddDebugLogLineN(logServerUDP,
				CFormat("Expected encrypted packet, but received unencrytped from server %s, "
					"UDPKey %u, Challenge: %u") %
					pServer->GetListName() % pServer->GetServerKeyUDP() %
					pServer->GetChallenge());
		} else {
			AddDebugLogLineN(logServerUDP,
				CFormat("Received encrypted packet from server %s, UDPKey %u, Challenge: "
					"%u") %
					pServer->GetListName() % pServer->GetServerKeyUDP() %
					pServer->GetChallenge());
		}
	...
	if (protocol == OP_EDONKEYPROT) {
		...
		AddDebugLogLineN(logServerUDP,
			CFormat("Received invalid packet, protocol (0x%x) and opcode (0x%x)") % protocol %
				opcode);
...
void CServerUDPSocket::ProcessPacket(CMemFile &packet, uint8 opcode, uint32 ip, uint16 port)
{
	...
	AddDebugLogLineN(logServerUDP,
		CFormat("Received UDP server packet from %s:%u, opcode (0x%x)") % Uint32toStringIP(ip) %
			port % opcode);
	...
	try {
		...
		case OP_GLOBSERVSTATRES: {
			...
			if (challenge != update->GetChallenge()) {
				throw wxString(CFormat("Invalid challenge on OP_GLOBSERVSTATRES packet (0x%x "
						       "!= 0x%x)") %
			...
			update->SetLastPingedTime(
				currentTime - (rand() % HR2S(1))); // if we used Obfuscated ping, we still
								   // need to reset the time properly
		...
		case OP_SERVER_DESC_RES: {
			...
			if (size >= 8 && Len == INV_SERV_DESC_LEN) {
				...
				if (update->GetDescReqChallenge() != 0 &&
					packet.ReadUInt32() == update->GetDescReqChallenge()) {
...
void CServerUDPSocket::OnReceiveError(int errorCode, uint32 ip, uint16 port)
{
	...
	if (pServer && !pServer->GetCryptPingReplyPending() &&
		GetTickCount64() - pServer->GetLastPinged() >= SEC2MS(30)) {
		pServer->AddFailedCount();
		Notify_ServerRefresh(pServer);
	}
}

void CServerUDPSocket::SendPacket(
	CPacket *packet, CServer *host, bool delPacket, bool rawpacket, uint16 port_offset)
{
	...
	if (!rawpacket && thePrefs::IsServerCryptLayerUDPEnabled() && host->GetServerKeyUDP() != 0 &&
		host->SupportsObfuscationUDP()) {
		...
		uRawPacketSize = CEncryptedDatagramSocket::EncryptSendServer(
			&pRawPacket, uRawPacketSize, host->GetServerKeyUDP());
		AddDebugLogLineN(logServerUDP,
			CFormat("Sending encrypted packet to server %s, UDPKey %u, port %u, original OPCode "
				"0x%02x") %
				host->GetListName() % host->GetServerKeyUDP() %
				host->GetObfuscationPortUDP() % packet->GetOpCode());
		...
		AddDebugLogLineN(logServerUDP,
			CFormat("Sending regular packet to server %s, port %u (raw = %s), OPCode 0x%02x") %
				host->GetListName() % host->GetObfuscationPortUDP() %
				(rawpacket ? "True" : "False") % packet->GetOpCode());
		if (delPacket) {
			item.packet = packet;
		} else {
...
void CServerUDPSocket::SendQueue()
{
	while (!m_queue.empty()) {
			...
			if (update) {
				...
				if (update->GetLastDNSSolve() + DNS_SOLVE_TIME < now) {
					...
					if (update->GetDNSError()) {
						...
						AddDebugLogLineN(logServerUDP,
							"Trying to send a UDP packet to a server host that "
							"failed DNS: " +
								item.addr);
						m_queue.pop_front();
						continue;
						...
						if (update->GetIP()) {
							...
							AddDebugLogLineN(logServerUDP,
								"Sending a UDP packet to a resolved DNS "
								"server host: " +
									item.addr);
							// Update the item IP
							...
							AddDebugLogLineN(logServerUDP,
								"Trying to send a UDP packet to a server "
								"host that is checking DNS: " +
									item.addr);
				...
				AddDebugLogLineN(logServerUDP,
					"Trying to send a UDP packet to a server host that is not on "
					"serverlist");
				// Not much we can do here, just drop the packet.
				m_queue.pop_front();
				continue;
		...
		if (update) {
			AddDebugLogLineN(
				logServerUDP, "Sending a UDP packet to a server: " + update->GetAddress());
			...
			AddDebugLogLineN(logServerUDP,
				"Sending a UDP packet to a server no in serverlist: " +
					Uint32_16toStringIP_Port(item.ip, item.port));
		}
