...
int CEncryptedDatagramSocket::DecryptReceivedClient(uint8_t *bufIn,
	int bufLen,
	uint8_t **bufOut,
	uint32_t ip,
	...
	uint32_t *senderVerifyKey)
{
	...
	*senderVerifyKey = 0;
	...
	uint32_t value = 0;
	// check the marker bit which type this packet could be and which key to test first, this is only an
	// indicator since old clients have it set random see the header for marker bits explanation
	...
	do {
		...
		if (currentTry == 0) {
			...
			if (Kademlia::CKademlia::GetPrefs()) {
				uint8_t keyData[18];
				Kademlia::CKademlia::GetPrefs()->GetKadID().StoreCryptValue(
					(uint8_t *)&keyData);
		...
		currentTry = (currentTry + 1) % 3;
		 ...
		 tries > 0); // try to decrypt as ed2k as well as kad packet if needed (max 3 rounds)
	...
	if (value == MAGICVALUE_UDP_SYNC_CLIENT) {
		...
		if (kad) {
			...
			receivebuffer.RC4Crypt(bufIn + CRYPT_HEADER_WITHOUTPADDING + padLen,
				(uint8_t *)receiverVerifyKey,
				4);
			receivebuffer.RC4Crypt(bufIn + CRYPT_HEADER_WITHOUTPADDING + padLen + 4,
				(uint8_t *)senderVerifyKey,
				4);
	...
	uint32_t senderVerifyKey)
{
	...
	MD5Sum md5;
	if (kad) {
		if ((clientHashOrKadID == NULL || CMD4Hash(clientHashOrKadID).IsEmpty()) &&
			receiverVerifyKey != 0) {
			kadRecvKeyUsed = true;
			uint8_t keyData[6];
	...
	for (i = 0; i < 128; i++) {
		...
		semiRandomNotProtocolMarker =
			kad ? (semiRandomNotProtocolMarker & 0xFE)
			    : (semiRandomNotProtocolMarker | 0x01); // set the ed2k/kad marker bit
		if (kad) {
			// set the ed2k/kad and nodeid/recvkey markerbit
			semiRandomNotProtocolMarker = kadRecvKeyUsed
							      ? ((semiRandomNotProtocolMarker & 0xFE) | 0x02)
							      : (semiRandomNotProtocolMarker & 0xFC);
		} else {
	...
	for (int j = 0; j < padLen; j++) {
		uint8_t byRand = (uint8_t)
			rand(); // they actually don't really need to be random, but it doesn't hurt either
	...
	if (kad) {
		ENDIAN_SWAP_I_32(receiverVerifyKey);
		ENDIAN_SWAP_I_32(senderVerifyKey);
		sendbuffer.RC4Crypt((uint8_t *)&receiverVerifyKey,
			cryptedBuffer + CRYPT_HEADER_WITHOUTPADDING + padLen,
			4);
		sendbuffer.RC4Crypt((uint8_t *)&senderVerifyKey,
			cryptedBuffer + CRYPT_HEADER_WITHOUTPADDING + padLen + 4,
			4);
	}
...
int CEncryptedDatagramSocket::DecryptReceivedServer(
	uint8_t *pbyBufIn, int nBufLen, uint8_t **ppbyBufOut, uint32_t dwBaseKey, uint32_t /*dbgIP*/)
{
	int nResult = nBufLen;
	*ppbyBufOut = pbyBufIn;

	if (nResult <= CRYPT_HEADER_WITHOUTPADDING || !thePrefs::IsServerCryptLayerUDPEnabled() ||
		dwBaseKey == 0) {
		return nResult;
	}
	...
	if (dwValue == MAGICVALUE_UDP_SYNC_SERVER) {
		...
		if (nResult <= byPadLen) {
			// DebugLogError(_T("Invalid obfuscated UDP packet from ServerIP: %s, Paddingsize (%u)
			// larger than received bytes"), ipstr(dbgIP), byPadLen);
			return nBufLen; // pass through, let the Receivefunction do the errorhandling on this
					// junk
		}
...
int CEncryptedDatagramSocket::EncryptSendServer(uint8_t **ppbyBuf, int nBufLen, uint32_t dwBaseKey)
{
	...
	for (int j = 0; j < byPadLen; j++) {
		uint8_t byRand = (uint8_t)
			rand(); // they actually don't really need to be random, but it doesn't hurt either
		sendbuffer.RC4Crypt(
			(uint8_t *)&byRand, pachCryptedBuffer + CRYPT_HEADER_WITHOUTPADDING + j, 1);
	}
