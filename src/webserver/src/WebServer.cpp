...
CDynPngImage::CDynPngImage(int w, int h)
: CAnyImage(w, h)
{
	...
	for (int i = 0; i < m_height; i++) {
		m_row_ptrs[i] = &m_img_data[static_cast<size_t>(3) * static_cast<size_t>(m_width) *
...
wxString CDynProgressImage::GetHTML()
{
	...
	for (int i = 0; i < m_width; i++) {
		if ((lastcolor != m_ColorLine[i]) || (i == (m_width - 1))) {
			...
			str += (CFormat(m_template) % progresscolor[color_idx] % (i - lastindex) %
				wxString(progresscolor[color_idx]).BeforeLast('.'))
				       .GetString();
			lastindex = i;
...
void CDynStatisticImage::DrawImage()
{
	...
	m_digits[0]->Apply(
		m_row_ptrs, 3 * img_delta + 2 * m_num_font_w_size, m_y_axis_size - m_num_font_h_size - 5);
	...
	for (int j = m_left_margin + 1, curr_data = m_data->GetNext(); j < m_width;
		j++, curr_data = m_data->GetNext()) {
		if (m_scale_down != 1) {
...
CImageLib::CImageLib(wxString image_dir)
...
char *CScriptWebServer::GetErrorPage(const char *message, long &size)
{
	...
	snprintf(buf, 1024, "<html><title> Error -%s </title></html>", message);
...
void CScriptWebServer::ProcessURL(ThreadData Data)
{
	...
	if (!session->m_logged_in && !public_asset) {
		...
		if (webInterface->m_AdminPass.IsEmpty() && webInterface->m_GuestPass.IsEmpty()) {
			...
			if (!PwHash.Decode(MD5Sum(PwStr).GetHash())) {
				...
				session->m_vars["login_error"] = "Invalid password hash, please report at "
	...
	session->m_vars["auto_refresh"] =
		(const char *)unicode2char(wxString(CFormat("%d") % webInterface->m_PageRefresh));
	...
	if (req_file.EndsWith(".html")) {
		...
		httpOut = ProcessHtmlRequest(unicode2char(req_file), httpOutLen);
	} else if (req_file.EndsWith(".dtd") || req_file.EndsWith(".xsd") || req_file.EndsWith(".xsl")) {
		session->m_vars["content_type"] = "text/xml";
		httpOut = ProcessHtmlRequest(unicode2char(req_file), httpOutLen);
	} else if (req_file.EndsWith(".appcache") || req_file.EndsWith(".manifest")) {
	...
	if (httpOut) {
		Data.pSocket->SendHttpHeaders(
			session->m_vars["content_type"].c_str(), isUseGzip, httpOutLen, Data.SessionID);
		Data.pSocket->SendData(httpOut, httpOutLen);
		delete[] httpOut;
	}
}

CNoTemplateWebServer::CNoTemplateWebServer(CamulewebApp *webApp)
: CScriptWebServer(webApp, "")
{
}
...
void CNoTemplateWebServer::ProcessURL(ThreadData Data)
{
	...
	const char *httpOut =
		...
		"</head>"
		"<body>"
		"<p>You are seeing this page instead of aMuleWeb login page because a valid template has not "
		"been found.</p>"
		"<p>This probably means that there's a problem with your aMule installation </p>"
		"<ul>"
		"<li>Before installing new versions, please ensure that you uninstalled older versions of "
		"aMule.</li>"
		"<li>If you are installing by compiling from source, check the configuration (e.g. that "
		"aMule was built with -DBUILD_WEBSERVER=YES) and re-run the install step.</li>"
		"<li>If you are installing by using a precompiled package, you may need to contact the "
		"package maintainer </li>"
		"</ul>"
		"<p>For more information please visit</p>"
		"<p><a href=\"https://github.com/amule-org/amule\">aMule on GitHub</a> or <a "
		"href=\"https://github.com/amule-org/amule/discussions\">aMule Discussions</a></p>"
		"</body>"
		"</html>";
