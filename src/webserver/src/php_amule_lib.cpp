...
void php_native_kad_update_from_url(PHP_VALUE_NODE *)
{
	...
	req.AddTag(CECTag(EC_TAG_KADEMLIA_UPDATE_URL, wxString(char2unicode(url))));
	CPhPLibContext::g_curr_context->WebServer()->Send_Discard_V2_Request(&req);
}
...
void php_native_add_server_cmd(PHP_VALUE_NODE *)
{
	...
	CPhPLibContext::g_curr_context->WebServer()->Send_AddServer_Cmd(
...
void php_get_amule_stats(PHP_VALUE_NODE *result)
{
	...
	const CECPacket *stats =
		CPhPLibContext::g_curr_context->WebServer()->webInterface->SendRecvMsg_v2(&stat_req);
	if (!stats) {
		return;
	}
	const CEC_ConnState_Tag *tag =
		static_cast<const CEC_ConnState_Tag *>(stats->GetTagByName(EC_TAG_CONNSTATE));
	if (!tag) {
		return;
	}
...
void php_get_amule_categories(PHP_VALUE_NODE *result)
{
	...
	const CECPacket *reply =
		CPhPLibContext::g_curr_context->WebServer()->webInterface->SendRecvMsg_v2(&req);
	if (!reply) {
		return;
	}
...
PHP_2_EC_OPT_DEF g_connection_opt_defs[] = { { "max_line_up_cap", EC_TAG_CONN_UL_CAP, 4 },
	{ "max_line_down_cap", EC_TAG_CONN_DL_CAP, 4 },
	{ "max_up_limit", EC_TAG_CONN_MAX_UL, 4 },
	{ "max_down_limit", EC_TAG_CONN_MAX_DL, 4 },
	{ "slot_alloc", EC_TAG_CONN_SLOT_ALLOCATION, 2 },
	{ "tcp_port", EC_TAG_CONN_TCP_PORT, 2 },
	{ "udp_port", EC_TAG_CONN_UDP_PORT, 2 },
	{ "udp_dis", EC_TAG_CONN_UDP_DISABLE, 0 },
	{ "max_file_src", EC_TAG_CONN_MAX_FILE_SOURCES, 2 },
	{ "max_conn_total", EC_TAG_CONN_MAX_CONN, 2 },
	{ "autoconn_en", EC_TAG_CONN_AUTOCONNECT, 0 },
	{ "reconn_en", EC_TAG_CONN_RECONNECT, 0 },
	{ "network_ed2k", EC_TAG_NETWORK_ED2K, 0 },
	{ "network_kad", EC_TAG_NETWORK_KADEMLIA, 0 },
	{ 0, (ECTagNames)0, 0 } };

PHP_2_EC_OPT_DEF g_file_opt_defs[] = { { "ich_en", EC_TAG_FILES_ICH_ENABLED, 0 },
	...
	{ "endgame", EC_TAG_FILES_ENDGAME, 0 },
...
PHP_2_EC_OPT_DEF g_webserver_opt_defs[] = { { "use_gzip", EC_TAG_WEBSERVER_USEGZIP, 0 },
	{ "autorefresh_time", EC_TAG_WEBSERVER_REFRESH, 4 },
	{ 0, (ECTagNames)0, 0 } };

PHP_2_EC_OPT_DEF g_coretweak_opt_defs[] = { { "max_conn_5sec", EC_TAG_CORETW_MAX_CONN_PER_FIVE, 2 },
	{ 0, (ECTagNames)0, 0 } };
...
void ec_tag_2_php(const CECTag *cattag, PHP_2_EC_OPT_DEF *opts, PHP_VAR_NODE *catvar)
{
	for (PHP_2_EC_OPT_DEF *def = opts; def->php_name; def++) {
		...
		switch (def->opsize) {
		case 0:
			val = cattag->GetTagByName(def->tagname) ? 1 : 0;
			break;
		case 1:
			val = cattag->GetTagByNameSafe(def->tagname)->GetInt();
			break;
		case 2:
			val = cattag->GetTagByNameSafe(def->tagname)->GetInt();
			break;
...
void php_get_amule_options(PHP_VALUE_NODE *result)
{
	...
	if ((cattag = reply->GetTagByName(EC_TAG_PREFS_GENERAL)) != 0) {
		...
		key->value.str_val =
			strdup(unicode2UTF8(cattag->GetTagByNameSafe(EC_TAG_USER_NICK)->GetStringData()));
	}
...
bool php_2_ec_tag(CECTag *cattag, PHP_2_EC_OPT_DEF *opts, PHP_VALUE_NODE *catvar)
{
	for (PHP_2_EC_OPT_DEF *def = opts; def->php_name; def++) {
		...
		case 1:
			cattag->AddTag(CECTag(def->tagname, (uint8)opt_var->value.int_val));
			break;
		case 2:
			cattag->AddTag(CECTag(def->tagname, (uint16)opt_var->value.int_val));
			break;
		case 4:
			cattag->AddTag(CECTag(def->tagname, (uint32)opt_var->value.int_val));
			break;
		default:
			return false;
		}
...
void php_set_amule_options(PHP_VALUE_NODE *)
{
	...
	if (nick_node && nick_node->value.type == PHP_VAL_STRING && nick_node->value.str_val) {
		...
		generalPrefs.AddTag(CECTag(EC_TAG_USER_NICK, wxString::FromUTF8(nick_node->value.str_val)));
	...
	if (opt_group_array->value.type == PHP_VAL_ARRAY) {
		...
		CPhPLibContext::g_curr_context->WebServer()->webInterface->m_UseGzip =
			pref->value.int_val != 0;
		pref = array_get_by_str_key(&opt_group_array->value, "autorefresh_time");
		cast_value_dnum(&pref->value);
		CPhPLibContext::g_curr_context->WebServer()->webInterface->m_PageRefresh =
			pref->value.int_val;
	}
	CPhPLibContext::g_curr_context->WebServer()->Send_Discard_V2_Request(&req);
}
...
void php_native_search_start_cmd(PHP_VALUE_NODE *)
{
	...
	CPhPLibContext::g_curr_context->WebServer()->Send_Search_Cmd(wxString(char2unicode(search)),
		wxString(char2unicode(ext)),
		wxString(char2unicode(type)),
		search_type,
		avail,
		min_size,
		max_size);
...
void amule_load_stats_tree(PHP_VALUE_NODE *result)
{
	...
	const CEC_StatTree_Node_Tag *stats_root =
		static_cast<const CEC_StatTree_Node_Tag *>(response->GetTagByName(EC_TAG_STATTREE_NODE));
	if (!stats_root) {
		delete response;
		return;
	}
...
void amule_download_file_prop_get(void *ptr, char *prop_name, PHP_VALUE_NODE *result)
{
	...
	if (strcmp(prop_name, "name") == 0) {
		...
		result->type = PHP_VAL_STRING;
		wxString short_name(
			obj->sFileName.Length() > 60 ? (obj->sFileName.Left(60) + (" ...")) : obj->sFileName);
...
void amule_upload_file_prop_get(void *ptr, char *prop_name, PHP_VALUE_NODE *result)
{
	...
	if (strcmp(prop_name, "name") == 0) {
		...
		// uploading file we don't share ?! We are either out of sync with core or a shared file has
		// been removed while uploading it
		if (!sharedfile) {
			SharedFile::GetContainerInstance()->ReQuery();
			sharedfile = SharedFile::GetContainerInstance()->GetByID(obj->nUploadFile);
		}
		result->str_val =
			strdup(sharedfile ? (const char *)unicode2UTF8(sharedfile->sFileName) : "???");
	} else if (strcmp(prop_name, "short_name") == 0) {
		result->type = PHP_VAL_STRING;
		SharedFile *sharedfile = SharedFile::GetContainerInstance()->GetByID(obj->nUploadFile);
		if (!sharedfile) {
		...
		wxString short_name;
		if (sharedfile) {
			short_name = sharedfile->sFileName.Length() > 60
					     ? (sharedfile->sFileName.Left(60) + (" ..."))
					     : sharedfile->sFileName;
		} else {
			short_name = "???";
		}
		result->str_val = strdup((const char *)unicode2UTF8(short_name));
