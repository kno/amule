...
PHP_SYN_NODE *make_ifelse_syn_node(
	PHP_EXP_NODE *expr, PHP_SYN_NODE *then_node, PHP_SYN_NODE *elseif_list, PHP_SYN_NODE *else_node)
{
	PHP_SYN_NODE *syn_node = new PHP_SYN_NODE;
	memset(syn_node, 0, sizeof(PHP_SYN_NODE));
...
PHP_SYN_NODE *make_for_syn_node(
	PHP_EXP_NODE *start, PHP_EXP_NODE *cond, PHP_EXP_NODE *next, PHP_SYN_NODE *code)
{
	PHP_SYN_NODE *syn_node = new PHP_SYN_NODE;
	memset(syn_node, 0, sizeof(PHP_SYN_NODE));
...
PHP_SYN_NODE *make_foreach_loop_syn_node(
	PHP_EXP_NODE *elems, PHP_EXP_NODE *i_key, PHP_EXP_NODE *i_val, PHP_SYN_NODE *code, int byref)
{
	PHP_SYN_NODE *syn_node = new PHP_SYN_NODE;
	memset(syn_node, 0, sizeof(PHP_SYN_NODE));
...
PHP_SYN_NODE *make_switch_syn_node(PHP_EXP_NODE *cond, PHP_EXP_NODE *case_list)
{
	...
	for (PHP_EXP_NODE *cur_case = case_list; cur_case; cur_case = cur_case->next) {
		...
		if (stat_list_tail) {
			while (stat_list_tail->next_node)
				stat_list_tail = stat_list_tail->next_node;
...
void func_scope_init(PHP_FUNC_PARAM_DEF *params,
	int param_count,
	PHP_SCOPE_TABLE_TYPE * /*scope_map*/,
	...
	for (int i = 0; i < param_count; i++) {
		...
		if (curr_arg_val->value.type != PHP_VAL_NONE) {
			if ((curr_arg_val->flags & PHP_VARFLAG_BYREF) || params[i].byref) {
				call_params[i] =
					php_expr_eval_lvalue((PHP_EXP_NODE *)curr_arg_val->value.ptr_val);
				if (!call_params[i]) {
...
void array_remove_at_str_key(PHP_VALUE_NODE *array, const std::string &key)
{
	...
	if (arr_ptr->array.count(key)) {
		...
		for (PHP_ARRAY_KEY_ITER_TYPE i = arr_ptr->sorted_keys.begin();
			i != arr_ptr->sorted_keys.end();
			++i) {
			if (*i == key) {
...
void value_value_assign(PHP_VALUE_NODE *dst, PHP_VALUE_NODE *src)
{
	...
	case PHP_VAL_ARRAY: {
		...
		for (PHP_ARRAY_KEY_ITER_TYPE i = src_array->sorted_keys.begin();
			i != src_array->sorted_keys.end();
			++i) {
...
void cast_value_str(PHP_VALUE_NODE *val)
{
	...
	case PHP_VAL_BOOL:
	case PHP_VAL_INT:
		snprintf(buff, sizeof(buff), "%" PRId64, val->int_val);
		break;
	case PHP_VAL_FLOAT:
		snprintf(buff, sizeof(buff), "%.02f", val->float_val);
		break;
	case PHP_VAL_STRING:
		return;
	case PHP_VAL_ARRAY: {
...
void php_exp_tree_free(PHP_EXP_NODE *tree)
{
	...
	case PHP_OP_FUNC_CALL: {
		...
		for (PHP_ARRAY_ITER_TYPE i = ((PHP_ARRAY_TYPE *)args_array->ptr_val)->array.begin();
			i != ((PHP_ARRAY_TYPE *)args_array->ptr_val)->array.end();
			++i) {
...
static void exp_set_ref(PHP_EXP_NODE *expr, PHP_VAR_NODE *var, PHP_VALUE_NODE *key)
{
	...
	default:
		php_report_error(PHP_ERROR, "Bad left part of operator =&: (%d)", expr->tree_node.left->op);
	}
}
...
void php_expr_eval(PHP_EXP_NODE *expr, PHP_VALUE_NODE *result)
{
	...
	case PHP_OP_CAT:
		...
		if (result) {
			...
			result->str_val = (char *)malloc(
				strlen(result_val_left.str_val) + strlen(result_val_right.str_val) + 1);
	...
	case PHP_OP_OBJECT_DEREF: // $x->y
		...
		if (get_scope_item_type(g_global_scope, lval_node->value.obj_val.class_name) ==
			PHP_SCOPE_NONE) {
		...
		if ((expr->tree_node.right->op != PHP_OP_VAL) ||
			(expr->tree_node.right->val_node.type != PHP_VAL_STRING)) {
		...
		if (si->class_decl->class_decl->is_native) {
			si->class_decl->class_decl->native_prop_get_ptr(lval_node->value.obj_val.inst_ptr,
				expr->tree_node.right->val_node.str_val,
				result);
		} else {
		...
		case PHP_ST_SWITCH: {
			...
			while (curr) {
				...
				if (cmp_result.int_val) {
					...
					cur_exec = static_cast<PHP_SYN_NODE *>(
						curr->exp_node->tree_node.syn_right);
					break;
				}
				curr = curr->next;
			}
