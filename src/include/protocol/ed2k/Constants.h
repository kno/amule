...
#define UDPSERVSTATREASKTIME \
	HR2S(4.5) // 4.5 hours - server last ping timestamp. Stored on server.met, which follows the eMule
		  // format (seconds from epoch)
...
#define MIN_UP_CLIENTS_ALLOWED \
	2 // min. clients allowed to download regardless UPLOAD_CLIENT_DATARATE or any other factors. Don't
	  // set this too high
...
// This 'identifier' is used for referencing shared part (incomplete) files with the OP_ASKSHAREDDIRS and
// related opcodes it was introduced with eDonkeyHybrid and is considered as part of the protocol.
