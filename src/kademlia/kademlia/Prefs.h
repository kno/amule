...
public:
	...
	{
		return m_lastContact ? ((time(NULL) - m_lastContact) < KADEMLIADISCONNECTDELAY) : false;
	}
	void SetLastContact() noexcept { m_lastContact = time(NULL); }
	bool HasLostConnection() const noexcept
	{
		return m_lastContact ? !((time(NULL) - m_lastContact) < KADEMLIADISCONNECTDELAY) : false;
	}
	...
	{
		firewalled ? ++m_statsUDPFirewalledNodes : ++m_statsUDPOpenNodes;
	}
	void StatsIncTCPFirewalledNodes(bool firewalled) noexcept
	{
		firewalled ? ++m_statsTCPFirewalledNodes : ++m_statsTCPOpenNodes;
	}
