...
#ifndef __WINDOWS__
/* XPM */
static const char *convert_xpm[] = { "16 16 9 1",
	" 	c None",
	".	c #B20000",
	"+	c #FF0000",
	"@	c #FF7F7F",
...
void CPartFileConvertDlg::UpdateProgress(float percent, wxString text, wxString header)
{
	if (s_convert_gui) {
		...
		wxStaticText *percentlabel =
			dynamic_cast<wxStaticText *>(s_convert_gui->FindWindow(IDC_CONV_PROZENT));
		percentlabel->SetLabel(buffer);

		if (!text.IsEmpty()) {
			dynamic_cast<wxStaticText *>(s_convert_gui->FindWindow(IDC_CONV_PB_LABEL))
				->SetLabel(text);
		}
...
void CPartFileConvertDlg::UpdateJobInfo(ConvertInfo &info)
{
	if (s_convert_gui) {
		...
		if (item_nr == -1) {
			item_nr = s_convert_gui->m_joblist->InsertItem(
				s_convert_gui->m_joblist->GetItemCount(), info.folder.GetPrintable());
			if (item_nr != -1) {
		...
		// update columns
		if (item_nr != -1) {
			s_convert_gui->m_joblist->SetItem(item_nr,
				0,
				info.filename.IsOk() ? info.folder.GetPrintable()
						     : info.filename.GetPrintable());
			s_convert_gui->m_joblist->SetItem(item_nr, 1, GetConversionState(info.state));
			if (info.size > 0) {
				s_convert_gui->m_joblist->SetItem(item_nr,
					2,
					CFormat(_("%s (Disk: %s)")) % CastItoXBytes(info.size) %
						CastItoXBytes(info.spaceneeded));
			} else {
...
void CPartFileConvertDlg::OnAddFolder(wxCommandEvent &WXUNUSED(event))
{
	...
	if (!folder.IsEmpty()) {
		int reply = wxMessageBox(
			_("Do you want the source files of successfully imported downloads be deleted?"),
			_("Remove sources?"),
...
void CPartFileConvertDlg::RemoveSel(wxCommandEvent &WXUNUSED(event))
{
	...
	for (long item_nr = m_joblist->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED); item_nr != -1;
