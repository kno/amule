 ...
 *
 * The wxDataViewCtrl counterpart of CMuleVirtualListCtrl, and virtual in the same sense: the
 * control materialises nothing per row. Rows are addressed by index through a
 * wxDataViewVirtualListModel, and cell text is pulled from the subclass on demand for the cells
 * actually drawn.
 *
 * Two consequences are worth knowing before deriving from this. The list owns the order: sorting is
 * done here, over m_items, rather than by wx calling wxDataViewModel::Compare() -- the columns are
 * marked sortable so the header caret appears, but wx is never asked to reorder anything.
 *
 * And item identity is not row identity. A wxDataViewItem from a row-addressed model encodes the
 * row number, so removing a row silently moves what any previously-held item refers to. Everything
 * below therefore speaks in wxUIntPtr item data, and selection is preserved across mutations by re-
 * resolving that data to rows -- never by holding wxDataViewItems.
 */
...
public:
	...
	/**
	 * Appends one item and shows it immediately, without sorting. For the single add that has
	 * to appear while a popup menu is up: the batch path defers its work, and idle events do
	 * not run inside a modal loop, so the row would otherwise not appear until the menu closed.
	 */
	...
	/**
	 * Drops many rows as one operation. A loop of RemoveItemData() costs an index rebuild and a
	 * selection round-trip per item; this pays each once, and tells the control about the whole
	 * set in a single notification.
	 */
	...
	/**
	 * The rows the control currently has on screen, as the inclusive range [firstRow, lastRow].
	 * lastRow is the last *fully* visible row, so a caller that must not miss a drawn pixel has
	 * to allow for one more below it.
	 *
	 * Returns false when the backend cannot report its viewport -- the wxDataViewCtrl base
	 * answers GetTopItem()/GetCountPerPage() with "don't know" and only the ports override
	 * them. The range is then meaningless and the caller has to fall back to whatever is safe
	 * for it. Which way that falls is not symmetric: treating a visible row as off screen
	 * leaves a stale cell in front of the user, whereas treating an off-screen row as visible
	 * only costs the work it saved.
	 */
	bool GetVisibleRowRange(long &firstRow, long &lastRow) const;
	/**
	 * Whether this item is part of the selection. Answers the one question without
	 * materialising the whole selection the way GetSelectedItemData() has to -- worth the
	 * separate entry point on the per-item update paths, which ask it once per changed row.
	 */
	bool IsItemDataSelected(wxUIntPtr data) const;
	wxUIntPtr ItemAt(long row) const;
	/**
	 * Scroll so that @p data sits at the top of the viewport again. Anchored to the row's data
	 * rather than to a wxDataViewItem: in a virtual list an item is a row number, which names a
	 * different row once a sort has moved things, so it cannot survive the rebuild. Does
	 * nothing for 0 or for data no longer in the list, which is what a row that has since gone
	 * away should do.
	 */
	...
	/**
	 * Resolves a wxDataViewItem from an event (an activation or context-menu event, say) back
	 * to its row, through the wxDataViewVirtualListModel AssociateVirtualModel() installed. A
	 * wxDataViewItem's ID *is* the row number for this model, but that detail is model-
	 * internal; going through GetRow() rather than casting the ID directly is what stays
	...
	/**
	 * Live text filter. Setting it calls RebuildFilteredView(), which the list implements by
	 * re-populating itself from its own data source -- the filter is purely a GUI-side view,
	 * exactly as on CMuleVirtualListCtrl.
	 */
...
protected:
	...
	/**
	 * Icon for a cell in a column appended as an icon+text column. Return false to draw text
	 * alone.
	 */
	...
	/**
	 * Fill data for a cell in a column appended with CMuleDataViewCtrl::AppendBarColumn(). The
	 * default leaves `out` empty, which CMuleBarRenderer draws as an empty bar -- fine for any
	 * column that never asks for one.
	 */
	...
	/**
	 * Whether the active sort column's values change while the list is up, so a refreshed row
	 * may have to move. Gated by thePrefs::LiveListSort() exactly as CMuleVirtualListCtrl gates
	 * it.
	 */
...
private:
	 ...
	 * reorder, and it is held off while the user is interacting so rows do not slide out from
	 * under the pointer -- the same treatment CMuleVirtualListCtrl gives it.
	 */
	void ScheduleResort();
	void MaybeResortNow();
