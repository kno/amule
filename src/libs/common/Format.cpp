...
inline bool isTypeChar(wxChar c)
{
	...
	case 'm': // (Glibc extension.)  Print output of strerror(errno).  No argument is required.
		  //		case '%':		// A `%' is written.  No argument is converted.  The
...
inline bool isFlagChar(wxChar c)
{
	...
	// glibc 2.2
	case 'I': // For decimal integer conversion (i, d, u) the output uses the locale's alternative output
		  // digits, if any.
		return true;
	}
...
{
	return ((str == "hh") || (str == "h") || (str == "l") || (str == "ll") || (str == "L") ||
		(str == "z") || (str == "j") || (str == "t") || (str == "q"));
}
	...
	// Extract format-string-like substrings from the input
	{
		...
		for (size_t pos = 0; pos < str.length(); ++pos) {
			...
			case esInvalidFormat:
				wxFAIL_MSG("Invalid format specifier: " +
					   str.Mid(formatStart, pos - formatStart + 1));
			/* fall through */
			case esFormatStart:
				formatStart = pos;
				break;
		...
		wxASSERT_MSG((state == esFormatEnd) || (state == esNonFormat),
			"Incomplete format specifier: " + str.Mid(formatStart));
	}
	...
	for (FormatList::iterator it = m_formats.begin(); it != m_formats.end();) {
		if (it->result == "%%") {
			...
			for (size_t pos = 1; pos < it->result.length(); ++pos) {
				...
				case efType:
					...
					if (c == 'm') {
						...
						do {
							...
							*buf = '\0';
							...
							if ((result == 0) || (buflen > 1024)) {
								ProcessArgument<const wxString &>(
									it, wxString(buf, wxConvLibc));
							} else if (errno == EINVAL) {
								if (*buf == '\0') {
									ProcessArgument<const wxString &>(it,
										wxString::Format(
											_("Unknown error %d"),
											errnum));
								} else {
									ProcessArgument<const wxString &>(it,
										wxString(buf, wxConvLibc));
								}
							} else if (errno != ERANGE) {
								ProcessArgument<const wxString &>(it,
									wxString::Format(
										_("Unable to get error "
										  "description for error %d"),
										errnum));
							} else {
								buflen <<= 1;
							}
						...
						wxFAIL_MSG(wxString::Format(
							"Unable to get error description for error %d.",
							errnum));
						ProcessArgument<const wxString &>(it,
							wxString::Format(_("Unable to get error description "
									   "for error %d"),
								errnum));
#endif
					} else {
				...
				wxCHECK2_MSG(
					state != efError, break, "Invalid format specifier: " + it->result);
				if (state == efType) {
...
template <> void CFormat::ProcessArgument(FormatList::iterator it, double value)
{
	switch (it->type) {
	case 'a':
	case 'A':
...
template <> void CFormat::ProcessArgument(FormatList::iterator it, wxChar value)
{
	switch (it->type) {
	case 'c':
		break;
...
template <> void CFormat::ProcessArgument(FormatList::iterator it, signed long long value)
{
	switch (it->type) {
	case 'c':
...
template <> void CFormat::ProcessArgument(FormatList::iterator it, unsigned long long value)
{
	switch (it->type) {
	case 'c':
		ProcessArgument(it, (wxChar)value);
...
template <> void CFormat::ProcessArgument(FormatList::iterator it, const wxString &value)
{
	if (it->type != 's') {
	...
	if ((it->type == 'p') || (it->type == 's')) {
		...
		if (sizeof(void *) == 8) {
			...
			it->result = wxString::Format(
				wxString("0x%016") + WXLONGLONGFMTSPEC + "x", (uintptr_t)value);
		} else {
			// 32 bit
...
template <typename _Tp> CFormat &CFormat::operator%(_Tp value)
{
	m_argIndex++;
