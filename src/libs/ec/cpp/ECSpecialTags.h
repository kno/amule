...
// #warning Kry - Preferences packet derived from packet, and that's ok, but shouldn't be here because this is
// a tag file and forces a stupid include
...
{
public:
	CEC_Prefs_Packet(uint32 selection,
		EC_DETAIL_LEVEL prefs_detail = EC_DETAIL_FULL,
		EC_DETAIL_LEVEL cat_details = EC_DETAIL_FULL);
...
public:
	...
	wxString ServerVersion(wxString *target = 0) const
	{
	...
	uint32 GetMaxUsers(uint32 *target = 0) const
	{
...
{
public:
	CEC_SharedFile_Tag(const CKnownFile *file,
		EC_DETAIL_LEVEL detail_level,
		CValueMap *valuemap = NULL,
		ec_tagname_t name = EC_TAG_KNOWNFILE);
	...
	uint16 GetRequests(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_REQ_COUNT, target);
	}
	uint32 GetAllRequests(uint32 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_REQ_COUNT_ALL, target);
	}

	uint16 GetAccepts(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_ACCEPT_COUNT, target);
	}
	uint32 GetAllAccepts(uint32 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_ACCEPT_COUNT_ALL, target);
	}

	uint64 GetXferred(uint64 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_XFERRED, target);
	}
	uint64 GetAllXferred(uint64 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_XFERRED_ALL, target);
	}

	uint16 GetCompleteSourcesLow(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_COMPLETE_SOURCES_LOW, target);
	}
	uint16 GetCompleteSourcesHigh(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_COMPLETE_SOURCES_HIGH, target);
	}
	uint16 GetCompleteSources(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_COMPLETE_SOURCES, target);
	}
	...
	bool GetAICHHash(wxString &target) const
	{
		return AssignIfExist(EC_TAG_KNOWNFILE_AICH_MASTERHASH, target);
	}
...
public:
	...
	uint16 SourceCount(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_SOURCE_COUNT, target);
	}
	uint16 SourceNotCurrCount(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_SOURCE_COUNT_NOT_CURRENT, target);
	}
	uint16 SourceXferCount(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_SOURCE_COUNT_XFER, target);
	}
	uint16 SourceCountA4AF(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_SOURCE_COUNT_A4AF, target);
	}
	...
	time_t LastSeenComplete(time_t *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_LAST_SEEN_COMP, target);
	}
	time_t LastDateChanged(time_t *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_LAST_RECV, target);
	}
	uint32 DownloadActiveTime(uint32 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_DOWNLOAD_ACTIVE, target);
	}
	uint16 AvailablePartCount(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_AVAILABLE_PARTS, target);
	}
	...
	bool HashingProgress(uint16 &target) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_HASHED_PART_COUNT, target);
	}

	uint64 GetLostDueToCorruption(uint64 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_LOST_CORRUPTION, target);
	}
	uint64 GetGainDueToCompression(uint64 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_GAINED_COMPRESSION, target);
	}
	uint32 TotalPacketsSavedDueToICH(uint32 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_SAVED_ICH, target);
	}
...
public:
	...
	uint64 XferDown(uint64 *target = 0) const
	{
		return AssignIfExist(EC_TAG_CLIENT_DOWNLOAD_TOTAL, target);
	}
	uint64 XferUpSession(uint64 *target = 0) const
	{
		return AssignIfExist(EC_TAG_CLIENT_UPLOAD_SESSION, target);
	}
	uint64 XferDownSession(uint64 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_SIZE_XFER, target);
	}
	...
	bool ClientUploadState(uint8 &target) const
	{
		return AssignIfExist(EC_TAG_CLIENT_UPLOAD_STATE, target);
	}
	bool ClientDownloadState(uint8 &target) const
	{
		return AssignIfExist(EC_TAG_CLIENT_DOWNLOAD_STATE, target);
	}

	// uint32 WaitTime(uint32 *target = 0) const { return AssignIfExist(EC_TAG_CLIENT_WAIT_TIME, target);
	// } uint32 XferTime(uint32 *target = 0) const { return AssignIfExist(EC_TAG_CLIENT_XFER_TIME,
	// target); } uint32 LastReqTime(uint32 *target = 0) const { return
	// AssignIfExist(EC_TAG_CLIENT_LAST_TIME, target); } uint32 QueueTime(uint32 *target = 0) const {
	// return AssignIfExist(EC_TAG_CLIENT_QUEUE_TIME, target); }
	...
	uint16 ServerPort(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_CLIENT_SERVER_PORT, target);
	}
	wxString ServerName(wxString *target = 0) const
	{
		return AssignIfExist(EC_TAG_CLIENT_SERVER_NAME, target);
	}
	...
	uint16 WaitingPosition(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_CLIENT_WAITING_POSITION, target);
	}
	uint16 RemoteQueueRank(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_CLIENT_REMOTE_QUEUE_RANK, target);
	}
	uint16 OldRemoteQueueRank(uint16 *target = 0) const
	{
		return AssignIfExist(EC_TAG_CLIENT_OLD_REMOTE_QUEUE_RANK, target);
	}

	EIdentState GetCurrentIdentState(EIdentState *target = 0) const
	{
		return (EIdentState)AssignIfExist(EC_TAG_CLIENT_IDENT_STATE, (uint32 *)target);
	}
	...
	bool NextRequestedPart(uint16 &target) const
	{
		return AssignIfExist(EC_TAG_CLIENT_NEXT_REQUESTED_PART, target);
	}
	bool LastDownloadingPart(uint16 &target) const
	{
		return AssignIfExist(EC_TAG_CLIENT_LAST_DOWNLOADING_PART, target);
	}
	...
	bool RemoteFilename(wxString &target) const
	{
		return AssignIfExist(EC_TAG_CLIENT_REMOTE_FILENAME, target);
	}
	bool DisableViewShared(bool &target) const
	{
		return AssignIfExist(EC_TAG_CLIENT_DISABLE_VIEW_SHARED, target);
	}
	...
	bool AvailableParts(uint16 &target) const
	{
		return AssignIfExist(EC_TAG_CLIENT_AVAILABLE_PARTS, target);
	}
...
public:
	...
	wxString FileName() const { return GetTagByNameSafe(EC_TAG_PARTFILE_NAME)->GetStringData(); }
	uint64 SizeFull() const { return GetTagByNameSafe(EC_TAG_PARTFILE_SIZE_FULL)->GetInt(); }
	uint32 SourceCount(uint32 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_SOURCE_COUNT, target);
	}
	uint32 CompleteSourceCount(uint32 *target = 0) const
	{
		return AssignIfExist(EC_TAG_PARTFILE_SOURCE_COUNT_XFER, target);
	}
	...
	{
		return AssignIfExist(EC_TAG_PARTFILE_STATUS, target);
	}
	bool GetRating(uint8 &target) const { return AssignIfExist(EC_TAG_KNOWNFILE_RATING, target); }
...
public:
	// search request
	CEC_Search_Tag(const wxString &name,
