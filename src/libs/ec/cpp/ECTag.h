...
public:
	...
	uint32 IP() { return m_ip[0] | (m_ip[1] << 8) | (m_ip[2] << 16) | (m_ip[3] << 24); }
	...
	std::string StringIPSTL(bool brackets = true)
	{
		...
		if (brackets)
			string_ip << "[";
		string_ip << (int)m_ip[0] << "." << (int)m_ip[1] << "." << (int)m_ip[2] << "." << (int)m_ip[3]
			  << ":" << m_port;
		if (brackets)
			string_ip << "]";
...
public:
	...
	const CECTag *GetFirstTagSafe() const
	{
		return m_tagList.empty() ? &s_theNullTag : &*m_tagList.begin();
	}
...
{
public:
	CECEmptyTag(ec_tagname_t name = 0)
	: CECTag(name, 0, (const void *)NULL)
	{
	}
};
...
{
public:
	CECIntTag(ec_tagname_t name, uint64 data)
	: CECTag(name, data)
	{
	}
};
