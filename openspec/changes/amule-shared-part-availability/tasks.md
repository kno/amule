...
- [x] 1.1 Add `PartBarSpansTest` to `unittests/tests/CMakeLists.txt`, mirroring
      `PartBarLegendTest` at `:1976-1993` (exec, `add_test`, `src` include,
      `muleunit`+`mulecommon`).
- [x] 1.2 **RED (decision 4's proof).** Create `src/PartBarSpans.h` with `SpansFor`
...
- [x] 1.3 **GREEN.** Change to `end = min(start + PARTSIZE, fileSize) - 1`
...
- [x] 1.4 **RED → GREEN** `ModeFor(hashingProgress, partCount)`: `partCount == 0` →
      `None` for every progress including `> 0`; `(4, 9)` → `Hashing` with the hashed
      count clamped to `4`.
- [x] 1.5 Implement per design's open question: `constexpr` index→span accessor plus a
      thin non-`constexpr` filler. No raw index loops — `modernize-loop-convert` has
      already failed this project's Tier-2 gate once.
- [x] 1.6 Fix `unittests/tests/CMakeLists.txt:1971-1975`. It describes "the `?` beside
...
- [x] 2.1 **RED** in `PartBarLegendTest.cpp`: fade as independent literals, never
...
- [x] 2.2 **GREEN** `kAvailFull = 10`, `lo`/`hi`, `SourceAvailabilityColour(n)` as
      `floor((2*(lo*9 + d*k) + 9) / 18)`, `k = min(n-1, kAvailFull-1)`.
- [x] 2.3 **RED → GREEN** availability palette + `AvailabilityPartColour`. Add distinct
...
- [x] 2.4 **RED → GREEN** `enum class SharedFilesBarColumn` + the
      ...
      preferences.
- [x] 2.5 Extend the per-legend uniqueness test (`PartBarLegendTest.cpp:152`) to the
      two new legends, shaded and flat.
- [x] 2.6 Exhaustive id test: all 15 shared-files `#define` ids and all 16
      `GenericColumnEnum` values, pinning the `5`/`6`/`8` cross-space collision a
      deliberate `static_cast` would still reach.
- [x] 2.7 Cross-surface pin: read `AVAIL_FULL = (\d+)` from
...
- [x] 3.1 Create `src/PartBarLegendUI.{h,cpp}`; move `MakeLegendSwatch`,
...
- [x] 3.2 `MakeGradientLegendSwatch()` — endpoints from `SourceAvailabilityColour(1)`
      ...
      here.
- [x] 3.3 `src/SharedFilesCtrl.cpp:658-678` adopts `SourceAvailabilityColour` and
      `SpansFor`; `:643-657` adopts `kBoth` and `kFlatHashPending`. Pixels move here.
- [x] 3.4 Row context-menu entry after `src/SharedFilesCtrl.cpp:260` + `EVT_MENU` row,
...
- [x] 4.1 **RED → GREEN** both-call-sites test: `n` in `{0,1,2,5,9,10,11,1000}` against
      ...
      arity are the guard.
- [x] 4.2 `src/DownloadListCtrl.cpp:1000-1001` → `SourceAvailabilityColour`; the
      misnamed `blue` local disappears with the duplicate. Spans stay gap-bounded
      (`:997-1010`) — `SpansFor` is **not** adopted here.
- [x] 4.3 Correct the comment at `src/webapi/static/js/components.js:384-388` (wrong
      channel, wrong saturation).

## Phase 5: Slice E — rename and i18n

- [x] 5.1 `src/SharedFilesCtrl.cpp:157` `_("Obtained Parts")` → `_("Source
      Availability")`.
- [x] 5.2 Assert the persistence letter string at `src/SharedFilesCtrl.cpp:182` is
...
- [x] 5.3 Run `./scripts/update-po.sh` (`po/amule.pot` + 40 catalogues).
- [x] 5.4 Verify idempotency: re-run and confirm zero `msgid`/`msgstr` lines change and
      only `POT-Creation-Date` moves; revert that churn before committing.
- [x] 5.5 `./scripts/check-potfiles.sh` MUST exit 0.
