#!/usr/bin/env bash
#
# amuleapi 02-auth — auth surface. Exercises /api/v1/auth/login,
...
_curl -H "Authorization: Bearer $TOKEN" "$API/auth/session"
...
_curl -b "amuleapi_token=$TOKEN" "$API/auth/session"
...
_curl -H "Authorization: Bearer not.a.real.jwt" "$API/auth/session"
...
# --- 9. Method gate. ----------------------------------------------
_curl -X GET "$API/auth/login?include_token=true"
_assert_status 405 "GET /auth/login → 405 method_not_allowed"
_curl -X GET "$API/auth/logout"
...
_curl -H "Authorization: Bearer $PW_TOKEN" "$API/auth/passwords"
...
_curl -H "Authorization: Bearer $NEW_TOKEN" "$API/auth/passwords"
...
_curl -X POST -H "Content-Type: application/json" \
	-d '{"password":"guestpass"}' "$API/auth/login?include_token=true"
...
_curl -H "Authorization: Bearer $GUEST_TOKEN" "$API/auth/passwords"
...
NEW_TOKEN=$(printf '%s' "$CURL_BODY" | jq -r .token)
_curl -X POST -H "Content-Type: application/json" \
	-d "{\"password\":\"$ADMIN_PASS\"}" "$API/auth/login?include_token=true"
_assert_status 401 "the replaced admin password no longer logs in → 401"
_curl -X POST -H "Content-Type: application/json" \
	-d '{"password":"rotated-pass"}' "$API/auth/login?include_token=true"
...
_curl -X POST "$API/auth/passwords"
...
for i in 1 2 3 4; do
	_curl -X POST -H "Content-Type: application/json" \
		-d '{"password":"wrong"}' \
		"$API/auth/login?include_token=true" > /dev/null
done
