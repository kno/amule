...
TEST(RangeMap, SingleInsert_BeforeStart_AtStart_InSide)
{
	for (int offset = 0; offset < 3; ++offset) {               // at start, inside, inside
		...
		wxString expected =
			wxString::Format("[(0, %u, 1), (%u, 150, 0)]", 100 + offset, 101 + offset);
	...
	for (int type = 0; type < 2; ++type) {
		...
		for (int end_offset = 0; end_offset < 3; ++end_offset) {
			...
			for (int start_offset = 0; start_offset < 2; ++start_offset) {
				wxString expected = wxString::Format(
					"[(%u, %u, %i)]", 98 + start_offset, 150 + end_offset, type);
...
TEST(RangeMap, SingleInsert_TouchingStart_AtStart_AtStart_Inside)
{
	for (int offset_a = 0; offset_a < 2; ++offset_a) {         // (touching start, at start)
		for (int offset_b = 0; offset_b < 2; ++offset_b) { // (at start, inside)
			...
			// Different
			wxString exp =
				wxString::Format("[(%u, %u, 1), (%u, 150, 0)]", start, end, 101 + offset_b);
	...
	for (int offset_a = 0; offset_a < 2; ++offset_a) {
		...
		for (int offset_b = 0; offset_b < 3; ++offset_b) {
			...
			singleInsert(start,
				...
				1,
				wxString::Format("[(100, %u, 0), (%u, %u, 1)]", 148 + offset_a, start, end));
		}
	}
}
...
TEST(RangeMap, MultiInsert_BeforeStart_BeforeStart)
{
	for (int type = 0; type < 2; ++type) { // Test with same and different type
		multiInsert(CONT,
			...
			90,
			type,
			wxString("[(0, 90, ") << type << "), (100, 150, 0), (151, 200, 1)]");
		multiInsert(SDIFF,
			0,
			90,
			type,
			wxString("[(0, 90, ") << type << "), (100, 150, 0), (160, 200, 1)]");
		multiInsert(SSAME,
			0,
			90,
			type,
			wxString("[(0, 90, ") << type << "), (100, 150, 1), (160, 200, 1)]");
	}
}
...
TEST(RangeMap, MultiInsert_BeforeStart_AtStart_InSide)
{
	for (int offset = 0; offset < 3; ++offset) { // (at start, inside, inside)
		...
		multiInsert(SSAME,
			...
			0,
			wxString::Format("[(0, %u, 0), (%u, 150, 1), (160, 200, 1)]", end, 101 + offset));

		multiInsert(CONT,
			0,
			end,
			1,
			wxString::Format("[(0, %u, 1), (%u, 150, 0), (151, 200, 1)]", end, 101 + offset));
		multiInsert(SDIFF,
			0,
			end,
			1,
			wxString::Format("[(0, %u, 1), (%u, 150, 0), (160, 200, 1)]", end, 101 + offset));
...
TEST(RangeMap, MultiInsert_BeforeStart_TouchingStart_AtEnd_TouchingEnd_AfterEnd)
{
	for (int end_offset = 0; end_offset < 3; ++end_offset) { // (at end, touching end, after end)
		for (int start_offset = 0; start_offset < 2;
			++start_offset) { // (before start, touching start)
			...
			multiInsert(CONT,
				...
				0,
				wxString::Format("[(%u, %u, 0), (%u, 200, 1)]", start, end, end + 1));
			multiInsert(SDIFF,
				start,
				end,
				0,
				wxString::Format("[(%u, %u, 0), (160, 200, 1)]", start, end));
			multiInsert(SSAME,
				start,
				end,
				0,
			...
			multiInsert(SDIFF,
				...
				1,
				wxString::Format("[(%u, %u, 1), (160, 200, 1)]", start, end));
			multiInsert(SSAME,
				start,
				end,
				1,
	...
	for (int offset_a = 0; offset_a < 2; ++offset_a) {         // (touching start, at start)
		for (int offset_b = 0; offset_b < 2; ++offset_b) { // (at start, inside)
			...
			multiInsert(CONT,
				...
				0,
				wxString::Format("[(%u, 150, 0), (151, 200, 1)]", start));
			multiInsert(SDIFF,
				start,
				end,
				0,
				wxString::Format("[(%u, 150, 0), (160, 200, 1)]", start));
			multiInsert(SSAME,
				start,
				end,
				0,
				wxString::Format(
					"[(%u, %u, 0), (%u, 150, 1), (160, 200, 1)]", start, end, end + 1));
			...
			multiInsert(CONT,
				...
				1,
				wxString::Format(
					"[(%u, %u, 1), (%u, 150, 0), (151, 200, 1)]", start, end, end + 1));
			multiInsert(SDIFF,
				start,
				end,
				1,
				wxString::Format(
					"[(%u, %u, 1), (%u, 150, 0), (160, 200, 1)]", start, end, end + 1));
			multiInsert(SSAME,
				start,
				end,
				1,
				wxString::Format("[(%u, 150, 1), (160, 200, 1)]", start));
		}
	}
}
...
TEST(RangeMap, MultiInsert_AtStart_AtEnd_TouchingEnd_AfterEnd)
{
	for (int offset = 0; offset < 3; ++offset) { // (at end, touching end, after end)
		...
		multiInsert(CONT,
			...
			wxString::Format("[(%u, %u, 0), (%u, 200, 1)]", start, end, end + 1));
		multiInsert(
			SDIFF, start, end, 0, wxString::Format("[(%u, %u, 0), (160, 200, 1)]", start, end));
		multiInsert(
			SSAME, start, end, 0, wxString::Format("[(%u, %u, 0), (160, 200, 1)]", start, end));

		multiInsert(CONT, start, end, 1, wxString::Format("[(%u, 200, 1)]", start));
		multiInsert(
			SDIFF, start, end, 1, wxString::Format("[(%u, %u, 1), (160, 200, 1)]", start, end));
		multiInsert(
			SSAME, start, end, 1, wxString::Format("[(%u, %u, 1), (160, 200, 1)]", start, end));
	}
}
	...
	for (int offset_a = 0; offset_a < 2; ++offset_a) {         // (inside, at end)
		for (int offset_b = 0; offset_b < 3; ++offset_b) { // (at end, touching end, after end)
			...
			multiInsert(CONT,
				...
				0,
				wxString::Format("[(100, %u, 0), (%u, 200, 1)]", end, end + 1));
			multiInsert(
				SDIFF, start, end, 0, wxString::Format("[(100, %u, 0), (160, 200, 1)]", end));
			multiInsert(SSAME,
				start,
				end,
				0,
				wxString::Format(
					"[(100, %u, 1), (%u, %u, 0), (160, 200, 1)]", start - 1, start, end));

			multiInsert(CONT,
				start,
				end,
				1,
				wxString::Format("[(100, %u, 0), (%u, 200, 1)]", start - 1, start));
			multiInsert(SDIFF,
				start,
				end,
				1,
				wxString::Format(
					"[(100, %u, 0), (%u, %u, 1), (160, 200, 1)]", start - 1, start, end));
			multiInsert(
				SSAME, start, end, 1, wxString::Format("[(100, %u, 1), (160, 200, 1)]", end));
		}
	}
}
...
TEST(RangeMap, MultiInsert_TouchingEnd_TouchingEnd_AfterEnd)
{
	for (int offset = 0; offset < 2; ++offset) { // (touching end, after end)
		...
		multiInsert(
			CONT, start, end, 0, wxString::Format("[(100, %u, 0), (%u, 200, 1)]", end, end + 1));
		multiInsert(SDIFF, start, end, 0, wxString::Format("[(100, %u, 0), (160, 200, 1)]", end));
		multiInsert(SSAME,
			start,
			end,
			0,
			wxString::Format("[(100, 150, 1), (%u, %u, 0), (160, 200, 1)]", start, end));
		...
		multiInsert(SDIFF,
			...
			1,
			wxString::Format("[(100, 150, 0), (%u, %u, 1), (160, 200, 1)]", start, end));
