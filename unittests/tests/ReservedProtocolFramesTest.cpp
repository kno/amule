...
// OP_UDPRESERVEDPROT2 (0xB2) carries no eD2k opcode: the byte after the protocol byte is a frame
// type, and the rest is that frame's payload (eMuleAI, srchybrid/ClientUDPSocket.cpp:982).
// Everything about that layout is wire format, including the fact that a datagram consisting of the
// protocol byte alone has no type byte to read.
//
// CClientUDPSocket needs theApp, so the classification is a free function in a header of its own
// and the socket does nothing but dispatch on the result. What this covers is therefore the part
// that decides: truncation, the five registered types, the payload window handed to each handler,
// and the unknown-type drop. The switch in CClientUDPSocket::ProcessReservedProt2Frame is a five-
// way jump over the same constants.
...
// Spec delta, "Truncated frame": an empty payload must be dropped and the read must not reach past
// the end. The classifier is handed the frame window only -- the byte after the protocol byte
// onwards -- so an empty window is exactly the datagram that carried nothing but 0xB2.
TEST(ReservedProtocolFrames, EmptyFrameIsTruncated)
{
	// A deliberately non-zero guard byte one position before the window: if the classifier read
	// backwards or past its length it would find 0xAA and report it as a frame type.
...
// Each registered type reaches its handler with the payload that follows it, and a type byte with
// nothing after it is a valid zero-length payload rather than truncation. Only the type byte itself
// is mandatory.
...
// Spec delta, "Frame type the client cannot handle": every type outside the registered five is
// dropped. Sweeping the whole byte range keeps a later type addition from silently landing in the
// wrong branch.
...
// The classifier is what the flood-accounting exemption rests on: it returns a disposition and
// touches nothing else, so a dropped frame cannot reach CPacketTracking. Only OP_KADEMLIAHEADER
// does (KademliaUDPListener.cpp:263), and this classifier has no way to get there. Asserting the
// function is const-correct over its input is the closest a unit test gets to that; the placement
...
// An unknown-frame flood must not become a log flood. The first occurrence logs, the rest of the
...
// A tick counter that appears to move backwards -- it should not, but the throttle must not answer
