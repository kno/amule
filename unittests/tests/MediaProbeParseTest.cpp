...
// Every fixture below is ffprobe's REAL output, captured verbatim from ffprobe 8.1.1 run with
// MediaProbe::ProbeEntries() and -of flat against a file generated for that case. Recording the
// output rather than the media keeps the test free of binary fixtures and of any ffmpeg dependency
// in CI, while still pinning the actual wire format -- including the parts easy to get wrong from
// reading the docs: the DISPOSITION: prefix, the mixed key case Matroska emits, and N/A for an
// absent duration.
...
// Cover art must never be taken for the file's codec (issue #1075). ffprobe reports embedded
// artwork as a regular video stream, so for an audio file it is the ONLY video stream and always
// won. The codec goes out on the wire to every peer, so this published "mjpeg" for a tagged music
// library.
...
{
	// One file, two cases: TAG:title lower, TAG:ALBUM and TAG:ARTIST upper. ffprobe matches the
	// requested names case-insensitively but prints the container's own case.
	MediaInfo info;
...
{
	// The audio streams here are labelled "Deutsch" and "Espanol" -- track names, not song
	// metadata. Falling back to stream tags on a video file would advertise one of them as the
	// file's title to every peer.
	MediaInfo info;
...
{
	// "duration=0.000000" parses, but a zero duration is not a duration. If this reported
	// success the caller would treat an all-empty MediaInfo as authoritative and CLEAR every
	// media tag the file had -- including a preview inherited from the search result -- then
	// re-probe it to the same effect on every subsequent start.
...
{
	// A one-track .mka muxed with --track-name 0:Deutsch passes every STRUCTURAL test -- one
	// audio stream, no video, nothing at format level -- so there is no way to tell its label
	// from a title. The fallback is therefore scoped to the containers that genuinely need it,
	// the Ogg family, whose comments live on the stream; an AAC track's stream tag is a label
...
{
	// The audio-only twin of the multi-track video case. A .mka or chained .ogg with two
	// labelled audio streams and no format-level tags has no video stream, so a "not a video"
	// test would let the first track's label through as the file's title. The fallback exists
	// for Ogg/Opus, where the comments live on the SINGLE logical stream -- so the condition is
...
// A crafted file must not be able to forge what we publish. Requesting format_tags / stream_tags
// puts attacker-controlled text into ffprobe's output for the first time -- container tags are
// arbitrary UTF-8 and may contain newlines. With the `default` writer, which does not escape
// values, each newline became another key=value line inside the tag's own section, and this parser
// is last-write-wins: a title of "Song\nduration=99999999" overwrote the real duration, and the
// forged value was attached as FT_MEDIA_LENGTH and published to every server and Kad node. `-of
// flat` escapes the value instead, and carries the section and stream index in the key so no value
// can forge a boundary either.
...
TEST(MediaProbeParse, CraftedTagCannotForgeDurationOrBitrate)
{
	...
	// And the payload lands where it belongs -- in the title -- with its newlines stripped by
	// the sanitiser, so it cannot impersonate several fields anywhere downstream either.
...
{
	// The same idea aimed at the structure rather than a value: a tag whose text looks like
	// another stream's key. With flat output the injected text stays inside the quoted value,
	// so it can neither start a stream nor overwrite one.
...
{
	// A container tag is arbitrary attacker-chosen text of arbitrary length, and it ends up in
	// known.met, in the log, and in every offered-file packet sent to every server and client.
	// The only bound downstream is the wire format's 0xFFFF truncation, which is a packet-
	// integrity guard, not a policy.
...
{
	// An ffprobe too old to know stream_disposition omits the line rather than failing: an
	// unknown FIELD is ignored, only an unknown SECTION is fatal. Such a stream must still be
	// eligible for codec selection.
...
{
	// The compound old-ffprobe case, recorded rather than argued about: with no DISPOSITION
	// line the artwork is indistinguishable from a real video track, so it wins the codec AND
	// -- because a non-empty videoCodec means the file is treated as a video -- suppresses the
	// stream-tag fallback. Such an ffprobe therefore degrades to exactly master's behaviour for
	// these files. Nothing can be done about it from this side; the test exists so the
	// degradation is a recorded property rather than a surprise.
